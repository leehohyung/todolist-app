...
| 백엔드 | Node.js, Express, JavaScript (ESM), JWT |
...
- **의존성:** 없음
- **Done When:**
  - [x] PostgreSQL 17 로컬 설치 및 실행 확인 (`psql --version`)
  - [x] `todolist_dev` DB 생성 (`createdb todolist_dev`)
  - [x] `todolist_test` DB 생성 (`createdb todolist_test`)
  - [x] `database/schema.sql` 적용 완료 (`psql -d todolist_dev -f database/schema.sql`)
  - [x] 기본 카테고리 3건(업무, 개인, 쇼핑) 삽입 확인 (`SELECT * FROM categories WHERE is_default = true` → 3 rows)
...
- **의존성:** 없음
- **Done When:**
  - [x] `backend/` 디렉터리 생성 및 `npm init` 완료
  - [x] nodemon 설치 (`npm i -D nodemon`)
  - [x] `tsconfig.json` 불필요 — JavaScript(CommonJS) 사용
  - [x] `package.json` scripts 구성: `dev`, `build`, `start`, `test`

#### BE-P1-02: 의존성 패키지 설치
- **의존성:** BE-P1-01
- **Done When:**
  - [x] 프로덕션 패키지 설치: `express pg bcrypt jsonwebtoken cors dotenv`
  - [x] 개발 패키지 설치: `nodemon`
  - [x] 테스트 패키지 설치: `jest supertest`
  - [x] `package.json`에 모든 패키지 반영 확인
...
- **Done When:**
  - [x] `.env.example` 파일 생성 (아래 변수 포함):
    ...
    ```
  - [x] `.env` 파일 생성 및 실제 값 설정 (`.gitignore`에 포함 확인)
  - [x] `src/config/index.ts` 작성: 환경변수 로드 및 필수값 누락 시 startup fail

#### BE-P1-05: 에러 코드 & 메시지 상수 정의
- **의존성:** BE-P1-01
- **Done When:**
  - [x] `src/constants/error-codes.ts` 작성:
    - `UNAUTHORIZED`, `INVALID_TOKEN`, `INVALID_INPUT`, `DUPLICATE_EMAIL`, `NOT_FOUND`, `FORBIDDEN`, `INTERNAL_SERVER_ERROR`
  - [x] `src/types/errors.ts` 작성: `AppError` 클래스 (code, statusCode, message)
...
- **의존성:** BE-P2-01
- **Done When:**
  - [x] `src/repositories/user-repository.ts` 작성
  - [x] 구현 메서드:
    - [x] `findByEmail(email): Promise<User | null>`
    - [x] `findById(userId): Promise<User | null>`
    - [x] `create(email, hashedPassword, name): Promise<User>`
    - [x] `updateName(userId, name): Promise<User>`
    - [x] `updatePassword(userId, hashedPassword): Promise<void>`
    - [x] `delete(userId): Promise<void>`
  - [x] 모든 쿼리 파라미터 바인딩 (`$1, $2, ...`) 사용 (문자열 연결 금지)
  - [x] DB 컬럼명(snake_case) → 반환 객체(camelCase) 변환은 Repository에서만 수행

### BE-P2-03: CategoryRepository 구현
- **의존성:** BE-P2-01
- **Done When:**
  - [x] `src/repositories/category-repository.ts` 작성
  - [x] 구현 메서드:
    - [x] `findAllForUser(userId): Promise<Category[]>` (기본 + 사용자정의 통합)
    - [x] `findById(categoryId): Promise<Category | null>`
    - [x] `findDefaultByName(name): Promise<Category | null>`
    - [x] `create(userId, name): Promise<Category>`
    - [x] `delete(categoryId): Promise<void>`
    - [x] `reclassifyTodos(fromCategoryId, toCategoryId): Promise<void>` (할일 카테고리 일괄 변경)
...
- **Done When:**
  ...
  - [x] 구현 메서드:
    - [x] `findById(todoId): Promise<Todo | null>`
    - [x] `findByIdAndUserId(todoId, userId): Promise<Todo | null>` (소유 검증)
    - [x] `findAllByUserId(userId, filters?): Promise<Todo[]>` (동적 필터 쿼리)
    - [x] `create(userId, data): Promise<Todo>`
    - [x] `update(todoId, data): Promise<Todo>`
    - [x] `updateCompletion(todoId, isCompleted): Promise<Todo>`
      - isCompleted=true → `completed_at = now()`
      - isCompleted=false → `completed_at = NULL`
    - [x] `delete(todoId): Promise<void>`
  - [x] 동적 쿼리(필터) 빌드 시 파라미터 바인딩 사용
...
- **의존성:** BE-P1-04
- **Done When:**
  - [x] `src/utils/jwt.js` 작성:
    - [x] `generateAccessToken(userId)` — 1시간 만료, JWT_SECRET
    - [x] `generateRefreshToken(userId)` — 7일 만료, JWT_REFRESH_SECRET
    - [x] `verifyAccessToken(token)` — 서명 검증, 페이로드 반환
    - [x] `verifyRefreshToken(token)` — 서명 검증, 페이로드 반환

### BE-P3-02: 비밀번호 유틸리티 작성
- **의존성:** BE-P1-01
- **Done When:**
  - [x] `src/utils/password.js` 작성:
    - [x] `hashPassword(password)` — bcrypt, saltRounds ≥ 10
    - [x] `comparePassword(plain, hashed)` — bcrypt compare

### BE-P3-03: 인증 미들웨어 구현
- **의존성:** BE-P3-01
- **Done When:**
  - [x] `src/middlewares/auth-middleware.js` 작성
  - [x] `Authorization: Bearer <token>` 헤더 파싱
  - [x] 토큰 유효 시 `req.userId` 설정 후 `next()` 호출
  - [x] 토큰 없음 → 401 `UNAUTHORIZED`
  - [x] 토큰 유효하지 않음 → 401 `INVALID_TOKEN`

### BE-P3-04: 에러 핸들러 미들웨어 구현
- **의존성:** BE-P1-05
- **Done When:**
  - [x] `src/middlewares/error-handler.js` 작성
  - [x] `AppError` 인스턴스 → statusCode + code + message 응답
  - [x] 일반 에러 → 500 `INTERNAL_SERVER_ERROR`
  - [x] 스택 트레이스 클라이언트 노출 금지
  - [x] 응답 형식: `{ error: { code, message } }`

### BE-P3-05: AuthService 구현
- **의존성:** BE-P2-02, BE-P3-01, BE-P3-02
- **Done When:**
  - [x] `src/services/auth-service.js` 작성
  - [x] 구현 메서드:
    - [x] `register(email, password, name)` — BR-01: 이메일 중복 확인, BR-02: 비밀번호 해싱
    - [x] `login(email, password)` — 이메일/비밀번호 검증, 토큰 쌍 발급
    - [x] `refreshTokens(refreshToken)` — 리프레시 토큰 검증 후 새 토큰 쌍 발급

### BE-P3-06: UserService 구현
- **의존성:** BE-P2-02, BE-P3-02
- **Done When:**
  - [x] `src/services/user-service.js` 작성
  - [x] `updateProfile(userId, { name?, currentPassword?, newPassword? })` 구현
    - [x] 이름 변경: name 제공 시 업데이트
    - [x] 비밀번호 변경: currentPassword 확인 후 newPassword 해싱 및 저장

### BE-P3-07: CategoryService 구현
- **의존성:** BE-P2-03
- **Done When:**
  - [x] `src/services/category-service.js` 작성
  - [x] 구현 메서드:
    - [x] `getCategoriesForUser(userId)` — 기본 카테고리 + 사용자 카테고리 통합 반환
    - [x] `createCategory(userId, name)` — 이름 유효성 검사 후 생성
    - [x] `deleteCategory(userId, categoryId)` — BR-07: 소유 검증, BR-08: 할일 '개인' 카테고리로 재분류 후 삭제

### BE-P3-08: TodoService 구현
- **의존성:** BE-P2-04, BE-P3-07
- **Done When:**
  - [x] `src/services/todo-service.js` 작성
  - [x] 구현 메서드:
    - [x] `createTodo(userId, data)` — BR-09: title/categoryId 필수 검증, 카테고리 소유 검증
    - [x] `updateTodo(userId, todoId, data)` — BR-10: 소유 검증 후 업데이트
    - [x] `deleteTodo(userId, todoId)` — BR-10: 소유 검증 후 삭제
    - [x] `toggleCompletion(userId, todoId)` — BR-11: isCompleted 토글, completedAt 자동 처리
    - [x] `getTodos(userId, filters)` — BR-13: 카테고리/날짜/완료 여부 필터 지원
    - [x] `calculateOverdueStatus(todo)` — BR-12: dueDate < today AND !isCompleted → 'overdue' (계산 속성)

### BE-P3-09: Controller 구현
- **의존성:** BE-P3-05, BE-P3-06, BE-P3-07, BE-P3-08
- **Done When:**
  - [x] `src/controllers/auth-controller.js` 작성: `register`, `login`, `refreshToken`
  - [x] `src/controllers/user-controller.js` 작성: `updateProfile`
  - [x] `src/controllers/category-controller.js` 작성: `getCategories`, `createCategory`, `deleteCategory`
  - [x] `src/controllers/todo-controller.js` 작성: `getTodos`, `createTodo`, `updateTodo`, `deleteTodo`, `toggleCompletion`
  - [x] 모든 응답 형식 통일:
...
- **Done When:**
  - [x] `src/routes/auth-routes.js` 작성:
    - [x] `POST /api/auth/register` (비인증)
    - [x] `POST /api/auth/login` (비인증)
    - [x] `POST /api/auth/refresh` (비인증)
  - [x] `src/routes/user-routes.js` 작성:
    - [x] `PATCH /api/users/me` (인증 필요)
  - [x] `src/routes/category-routes.js` 작성:
    - [x] `GET /api/categories` (인증 필요)
    - [x] `POST /api/categories` (인증 필요)
    - [x] `DELETE /api/categories/:categoryId` (인증 필요)
  - [x] `src/routes/todo-routes.js` 작성:
    - [x] `GET /api/todos` (인증 필요, 쿼리: `categoryId`, `dueDate`, `isCompleted`)
    - [x] `POST /api/todos` (인증 필요)
    - [x] `PATCH /api/todos/:todoId` (인증 필요)
    - [x] `DELETE /api/todos/:todoId` (인증 필요)
    - [x] `PATCH /api/todos/:todoId/complete` (인증 필요)

### BE-P3-11: Express 앱 & 서버 진입점 구현
- **의존성:** BE-P3-10, BE-P3-04
- **Done When:**
  - [x] `src/app.js` 작성:
    - [x] `express.json()` 미들웨어 등록
    - [x] CORS 미들웨어 등록 (origin = env CORS_ORIGIN)
    - [x] 라우터 마운트 (`/api/auth`, `/api/users`, `/api/categories`, `/api/todos`)
    - [x] 에러 핸들러 미들웨어 마지막에 등록
  - [x] `src/server.js` 작성:
    - [x] DB 연결 확인 후 서버 시작
    - [x] `PORT` 환경변수 기반 포트 바인딩
...
- **Done When:**
  - [x] `AuthService` 테스트: 회원가입(중복 이메일 처리), 로그인(비밀번호 검증), 토큰 갱신
  - [x] `CategoryService` 테스트: 카테고리 조회/생성/삭제(재분류 로직), 기본 카테고리 삭제 금지
  - [x] `TodoService` 테스트: CRUD, 완료 토글(completedAt), 기한 초과 계산, 필터링

### TEST-02: 백엔드 통합 테스트 (API E2E)
- **의존성:** BE-P3-11, DB-P1-01
- **Done When:**
  - [x] `POST /auth/register` — 정상 등록, 중복 이메일 409
  - [x] `POST /auth/login` — 정상 로그인, 잘못된 비밀번호 401
  - [x] `POST /auth/refresh` — 정상 갱신, 만료 토큰 401
  - [x] `GET /categories` — 기본+사용자 카테고리 통합 반환
  - [x] `DELETE /categories/:id` — 할일 재분류 후 삭제, 기본 카테고리 삭제 시 403
  - [x] `GET /todos` — 필터(카테고리, 날짜, 완료여부) 동작 확인
  - [x] `PATCH /todos/:id/complete` — isCompleted 토글, completedAt 자동 처리

### TEST-03: 비즈니스 규칙 검증
- **의존성:** TEST-02
- **Done When:**
  - [x] BR-01: 이메일 중복 등록 시 409 반환
  - [x] BR-02: 비밀번호 평문 DB 저장 안 됨 확인 (bcrypt 해시 확인)
  - [x] BR-05: 기본 카테고리(업무/개인/쇼핑) 항상 조회 가능
  - [x] BR-07: 타인 카테고리 삭제 시 403 반환
  - [x] BR-08: 카테고리 삭제 시 할일이 '개인' 카테고리로 이동
  - [x] BR-10: 타인 할일 수정/삭제 시 403 반환
  - [x] BR-11: 완료 처리 시 completedAt 기록, 취소 시 NULL
  - [x] BR-12: dueDate < today AND isCompleted=false → overdue 응답 필드 true
  - [x] BR-13: categoryId/날짜범위/isCompleted 필터 조합 정상 동작

### TEST-04: 프론트엔드 핵심 시나리오 수동 검증
- **의존성:** FE-P5-07, TEST-02
- **Done When:**
  - [x] SC-01 회원가입: 이메일/비밀번호/이름 입력 → 가입 완료 → 로그인 화면 이동
  - [x] SC-02 로그인: 이메일/비밀번호 입력 → 토큰 Zustand 메모리 저장 → 할일 목록 이동
  - [x] SC-03 로그아웃: 로그아웃 버튼 → 메모리 토큰 삭제 → 로그인 화면 이동
  - [x] SC-04 페이지 새로고침: 로그인 화면으로 이동 (토큰 초기화 확인)
  - [x] SC-05 할일 생성: 제목/카테고리 선택 → 저장 → 목록에 표시
  - [x] SC-06 할일 완료: 체크박스 클릭 → 취소선 표시, completedAt 기록
  - [x] SC-07 할일 필터: 카테고리/날짜/완료 여부 필터 → 결과 정상 반영
  - [x] SC-08 카테고리 삭제: 삭제 확인 다이얼로그 → 할일 '개인'으로 재분류 확인
  - [x] SC-09 기한 초과 표시: dueDate 지난 미완료 할일 → '기한 초과' 배지 표시
  - [ ] SC-10 반응형: 모바일(375px) / 태블릿(768px) / 데스크탑(1280px) 레이아웃 정상 확인 (브라우저 직접 확인 필요)
